#########Transfer scanVI assigned labels to human tanycyte object generated in 5_Human_tanycyte_unsupervised_subclustering to verify the assignments ###

library(Seurat)

# Load Seurat object
Tanycytes_human_clean <- readRDS(
  "/human_ME_full/Part2_Tany_human_subset/Tanycytes_human_clean.rds"
)

# Load scANVI metadata
scanvi_metadata <- read.csv(
  "/human_ME_full/Part3_cross_species/after_scvi/scanvi_metadata_full.csv",
  row.names = 1,
  check.names = FALSE
)
...
scanvi_latent <- read.table(
  "/human_ME_full/Part3_cross_species/after_scvi/scanvi_latent_full.txt",
  header = TRUE,
  sep = "\t",
  row.names = 1,
  check.names = FALSE
...
saveRDS(
  Tanycytes_human_scanvi,
  "/human_ME_full/Part3_cross_species/after_scvi/Tanycytes_human_with_scanvi.rds"
)

saveRDS(
  human, "/human_ME_full/Part3_cross_species/after_scvi/Tanycytes_human_with_scanvi.rds"
)
